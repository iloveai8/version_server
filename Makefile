...
deploy_stage:
	...
	@cd $(DeployPath)/overlays/$(ENV) \
		&& kustomize edit set annotation ver:$(VER) && kustomize edit add annotation kubesphere.io/description:"game slots version server $(ENV)-$(VER)" && kustomize edit set image $(HarborRegistry)/$(APP):$(ENV).$(VER) \
		&& cd - \
